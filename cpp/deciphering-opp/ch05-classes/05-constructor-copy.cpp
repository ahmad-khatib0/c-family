...
private:
  ...
  float gpa;
  char *currentCourse;
  // though we'll prefer std::string, this data member will
...
// default constructor
Student::Student() {
  // Remember, firstName and lastName are member objects of type string; they
  // are default constructed and hence 'empty' by default. They HAVE been
  // initialized.
  middleInitial = '\0';
  gpa = 0.0;
...
Student::Student(const string &fn, const string &ln, char mi, float avg,
                 const char *course) {
  firstName = fn;
  lastName = ln;
  middleInitial = mi;
