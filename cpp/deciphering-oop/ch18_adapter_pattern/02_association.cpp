...
public:
  ...
  const string &GetFirstName() const {
    return firstName;
  } // firstName returned as reference to const string
  ...
  virtual const string &Speak() {
    return greeting;
  } // note return type of const string & (we're no longer returning a literal)
...
// We'll see the need in a derived class below, so it is nice to review what the default
// implementation would look like Note: this version is equivalent to the default implementation,
// but we can easily see where you'd add more if so needed
Person &Person::operator=(const Person &p) {
  // make sure we're not assigning an object to itself
  if (this != &p) {
    // Note: there's no dynamically allocated data members, so implementing = is straightforward
    // If there were dynamicalled data members, you'd reallocate the memory to be the proper size in
    // the destination object, before copying from the source object. You'd also need to remember to
    // release the original memory in the destination object.
...
public:
  ...
  // virtual destructor -- note: class is NOT derived from Person
...
// Now, we do need to implement operator= ourselves. Let's see how
// (and thank goodness we reviewed basic implementation above)
...
const string &
Humanoid::Converse() // Yes, there can be a default implementation for a pure virtual function
...
public:
  ...
  // Orkan(const Orkan &h) = default;  // if we wrote copy constructor ourselves, we'd add :
  // Humanoid(h) { }  // copy constructor

  // Note: op= implementation is same as default op= at this level (but notice how we needed to
  // implement it ourselves at Humanoid level) Notice necessary downcast because Orkan::op= requires
  // derived type as return value
  ...
  // if we're using default ~Orkan() override = default;  // virtual destructor

  // We must override this method if we want Orkan to be a concrete class
...
public:
  ...
  // Remember, it's unnecessary to prototype default copy constructor
  // Romulan(const Romulan &h) = default;  // if we wrote copy constructor ourselves, we'd add :
  // Humanoid(h) { }  // copy constructor

  // Note: op= implementation is same as default op= at this level (but notice how we needed to
  // implement it ourselves at Humanoid level) Notice necessary downcast because Romulan::op=
  // requires derived type as return value
...
const string &Romulan::Converse() {
  return Humanoid::Converse(); // even if we just call the default implementation, we must provide a
                               // method here
}
...
public:
  ...
  // Remember, it's unnecessary to prototype default copy constructor
  Earthling(const Earthling &h) = default; // if we wrote copy constructor ourselves, we'd add :
                                           // Humanoid(h) { }  // copy constructor

  // Note: op= implementation is same as default op= at this level (but notice how we needed to
  // implement it ourselves at Humanoid level) Notice necessary downcast because earthling::op=
  // requires derived type as return value
  Earthling &operator=(const Earthling &h) {
    return dynamic_cast<Earthling &>(Humanoid::operator=(h));
  }

  // Since destructor has been indicated as virtual in base class, we don't need to protytpe it here
  // if we're using default ~Earthling() override = default;  // virtual destructor
...
int main() {
  ...
  // Add Humanoid *'s to the allies list. The list will make copies of the objects
  // internally and will properly clean up these objects when they are no longer members
  // of the list. You are responsible to delete and heap instances you provide to the
  // list when you are done with them.
  ...
  so we still need to derererence it
      (*entity).GetInfo();
      cout << (*entity).Converse() << endl;
  }
  */
  ...
  */
  ...
  // Though each type of Humanoid has a default Salutation, each may expand
  // their language skills and choose an alternate language
