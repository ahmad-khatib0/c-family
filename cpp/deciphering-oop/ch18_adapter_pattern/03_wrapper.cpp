...
public:
  ...
  // We will review the overloaded assignment operator (needed for upcoming example), so let's see
  // the prototype below in case we'd need to write it ourselves (of course, the default op=
  // suffices in this example, so this is commented out) Person &operator=(const Person &); //
...
// We're using the default, system-supplied copy constructor, but if you wrote it, it would look
// like:
/* Person::Person(const Person &p) : firstName(p.firstName), lastName(p.lastName),
                                  middleInitial(p.middleInitial), title(p.title),
greeting(p.greeting) { } */

// Here, let's just review how you'd overload the assignment operator in case you'd
// need to. We'll see such a scenario in an upcoming example, so it is nice to review
// what the default implementation would look like Note: this version is equivalent to
// the default implementation, but we can easily see where you'd add more if so needed
/*
Person &Person::operator=(const Person &p)
{
   ...
   if (this != &p)
   {
      ...
      // If there were dynamicalled data members, you'd reallocate the memory to be the proper size
in the destination object,
      // before copying from the source object. You'd also need to remember to release the original
memory in the destination object. firstName = p.firstName; lastName = p.lastName; middleInitial =
p.middleInitial; title = p.title; greeting = p.greeting;
   }
