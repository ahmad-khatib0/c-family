...
public:
  ...
  // The copy constructor prototype with =default is unnecessary - we get this
  // by automatically. Hence, it is commented out.
...
{
   // Remember, string members are automatically initialized to empty with the
...
// We are using default copy constructor, but if you needed to write it
// yourself, here is what it would look like: Remember to prototype it too!
/*
...
public:
  ...
  ~Student() override;      // This is a virtual destructor; override indicates
                            // 'entry' point in destruction sequence
  void EarnPhD();
  // inline function definitions
  ...
  // is recommended for internal documentation, however "virtual" is not
  // recommended
  // final indicates this may not be overridden beyond Student
...
void Student::Print()
    const { // need to use access functions as these data members are
...
int main() {
  ...
  // lets say that print in Student is: Print(const string &)
  // (dynamic_cast<Student *>(people[0]))->Print("Student");
  for (int i = 0; i < MAX; i++)
