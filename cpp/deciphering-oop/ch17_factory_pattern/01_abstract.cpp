...
// A disadvantage to this closely coupled implementation is that the abstract Product
// class must include a means for instantiation for each of its descendants in its static
// creation method, MatriculateStudent(). Adding new derived classes now affects the
// abstract base class definition – it needs to be recompiled. What if we don’t have
// access to the source code for this base class?
...
private:
  ...
  static int numStudents; // static data member is initialized outside of the class (see below)
...
// Notice that data members using in-class initialization (above), will be set for
// those members not in the member init list. However, those that can not be easily
// set with in-class initialization (such as static numStudents), we set below in method.
// Recall that member objects (strings) will be default constructed, so no additional
// init is necessary (if an empty string is our goal)
Student::Student() : studentId(to_string(numStudents + 100) + "Id") {
  // Note: since studentId is const, we need to set it at construction. We're doing
  // so in member init list with a unique id (based upon numStudents counter + 100),
  // concatenated with the string "Id" . Remember, string member currentCourse will
  // be default constructed with an empty string - it is a member object Also, remember
  // to dynamically allocate memory for any pointer data members here
...
public:
  ...
  // Since most base class has virtual specified on destructor, it is not necessary
  // at this level if you want the default ~GradStudent() override = default;
  void EarnPhD(); // not virtual because we don't want it overridden. This definition stands.
...
// We're using the default Copy constructor, but if we chose to write it ourselves:
...
public:
  ...
  // this level if you want the default ~UnderGradStudent() override = default;
...
void UnderGradStudent::Graduate() {
  // Verify that number of credits and gpa requirements have been met for major
  // and any minors or concentrations. Have all applicable university fees been paid?
...
public:
  ...
  // prototyping default copy constructor is not necessary
  // NonDegreeStudent(const NonDegreeStudent &s) = default; // copy constructor,
  // if we chose to write it, we'd add : Student(s) { } Since most base class has
  // virtual specified on destructor, it is not necessary at this level if you want the
  // default ~NonDegreeStudent() override = default; // destructor
...
void NonDegreeStudent::Graduate() {
  // Check if applicable tuition has been paid. There is no credit or gpa requirement.
...
int main() {
  ...
  scholars[0] =
      Student::MatriculateStudent("PhD", "Sara", "Kato", 'B', "Ms.", 3.9, "C++", "272PSU");
  scholars[1] = Student::MatriculateStudent("BS", "Ana", "Sato", 'U', "Ms.", 3.8, "C++", "178PSU");
  scholars[2] =
      Student::MatriculateStudent("None", "Elle", "LeBrun", 'R', "Miss", 3.5, "C++", "111BU");

  // for (int i = 0; i < MAX; i++)   // older looping style, see below for preferred style
  // {
  // scholars[i]->Graduate();
  // scholars[i]->Print();
  // }

  for (auto *oneStudent : scholars) // preferred looping style
  {
    oneStudent->Graduate();
    oneStudent->Print();
  }

  // less modern style for below loop
  // for (int i = 0; i < MAX; i++)
