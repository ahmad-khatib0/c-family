...
#include <iostream>
// data member to show several important concepts in this chapter (so we'll use
// a single char *)
...
void Student::Initialize() {
  // even though string data members, firstName and lastName, are initialized
  // with empty strings (string is a class and does so for us), we're showing
  // here how to clear the string, as it may be re-initialized more than once
  firstName.clear();
  lastName.clear();
...
void Student::Initialize(string fn, string ln, char mi, float avg,
                         const char *course) {
  firstName = fn;
  lastName = ln;
  middleInitial = mi;
  ...
  // remember to dynamically allocate the memory for data members that are
  // pointers
...
void Student::CleanUp() {
  ...
  // deallocate the memory for data members that are pointers
}
