...
// Note: The constructor member initialization list was originally created to provide
// a manner to initialize data members that are const or references. However, it can
// additionally (and preferably) be used as an efficient way to fully initialize
...
Student::Student()
    : firstName(), lastName(), middleInitial('\0'), gpa(0.0), currentCourse(), studentId(0) {
  // You may still set data members here, but using above initialization is more efficient than
  // using assignment Note: firstName, lastName, currentCourse are shown in member init. list
  // selecting default constructor for initialization. However, as this is the default action for
  // member objects (string), we don’t need to explicitly include these members in the member
  // initialization list (nor will we include them in future examples).
}
...
Student::Student(const string &fn, const string &ln, char mi, float avg, const string &course,
    ...
    : firstName(fn), lastName(ln), middleInitial(mi), gpa(avg), currentCourse(course),
      studentId(id) {
  // note: for string data members, the above initialization is calling the constructor
  // matching the signature of item in ()'s this is inevitably more efficient than default
  // constructing the data member and then assigning it with a new value in the method
  // body note: any pointer data members often need to be set within body of function
  // (but can sometimes be set in init. list too)
}
