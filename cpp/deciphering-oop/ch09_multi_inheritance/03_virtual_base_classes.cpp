...
// Virtual base classes resolve duplications which can exist with multiple
// inheritance when two or more base classes for a class share a common
// ancestor class.
...
// Here, LifeForm is a virtual base class of Horse. This means that if Horse is ever
// combined with a sibling (or cousin) of another base class which inherits from
// LifeForm, those siblings will SHARE their copy of LifeForm, reducing storage, extra
// constructor calls, and eliminating ambiguity. This will not at all affect instances
// of Horse (only instances of derived classes which would otherwise have duplicate
// LifeForm sub-objects).
...
// We are using the default (system supplied) copy constructor, but if we wanted to
// write it, it would look like: Remember to prototype it if you define it yourself.
...
public:
  ...
  // Because base class destructor is virtual, ~Person() is automatically virtual (overridden)
  // whether or not explicitly prorotyped ~Person() override = default;  // destructor
...
// Notice that the base class init list specification of LifeForm is ignored if
// LifeForm ACTUALLY is a shared virtual base class. Remember, string members
// are automatically initialized to empty with the default string constructor
...
private:
  // no additional data members required , but the below static constexpr
  // eliminates a magic number of 1000
...
public:
  ...
  Centaur(const string &,
          const string &,
          char = ' ',
          const string & = "Mythological Creature"); // note default args in alt ctor

  // Note: we do not want default copy constructor here,
  // due to virtual base class use in member init list
  Centaur(const Centaur &c) : Person(c), Horse(c), LifeForm(CENTAUR_LIFE) {}
  // notice initialization of shared base class object
  // Because base class' destructors are virtual, ~Centaur() is automatically virtual (overridden)
  // whether or not explicitly prorotyped ~Centaur() override = default;
  void Print() const override;
  string IsA() const override;
  string Speak() const override;
};

// Constructors for Centaur need to specify how the shared base class LifeForm will be
// initialized. This is only allowed because LifeForm is a virtual base class of Person
// and Horse.. So Without LifeForm as a virtual base class for Horse and Person, Centaur
// would not be able to specify how to construct the shared LifeForm piece
Centaur::Centaur(const string &fn, const string &ln, char mi, const string &title)
    : Person(fn, ln, mi, title), Horse(fn), LifeForm(CENTAUR_LIFE) {
...
string Centaur::Speak() const { return "Neigh! and Hello! I'm a master of two languages."; }
