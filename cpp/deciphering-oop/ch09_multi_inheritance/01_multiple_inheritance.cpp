...
// Simple destructor written ourselves, just so we can trace the destructor
// chain using cout statements
Person::~Person() {
...
public:
  ...
  Student(const string &, const string &, char, const string &, float,
          const string &, const string &, float);
...
// Due to non-specification in the member init list, this constructor calls the
// default base class constructors
Student::Student() : studentId(to_string(numStudents + 100) + "Id") {
  // Note: since studentId is const, we need to set it at construction. We're
  // doing so in member init list with a unique id (based upon numStudents
  // counter + 100), concatenated with the string "Id" . Remember, string member
  // currentCourse will be default constructed with an empty string - it is a
  // member object Also, remember to dynamically allocate memory for any pointer
  // data members here (not needed in this example)
  numStudents++;
}

// The member initialization list specifies which versions of each base class
// constructor should be utilized.
Student::Student(const string &fn, const string &ln, char mi, const string &t,
                 float avg, const string &course, const string &id, float amt)
    : Person(fn, ln, mi, t), BillableEntity(amt), gpa(avg),
...
Student::~Student() {
  numStudents--;
  cout << "Student destructor <" << GetFirstName() << " " << GetLastName()
       << ">" << endl;
}
...
int main() {
  ...
  // public members of Person, BillableEntity, Student are accessible from any
