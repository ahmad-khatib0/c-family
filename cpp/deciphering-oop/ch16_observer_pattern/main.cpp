...
// In the Observer pattern, an object of interest will maintain a list of
// observers who are interested in state updates of the main object.
...
// More enumerators within the states are created here than are currently used;
// they exist for expansion of example Note that more specialized states are based
// on a more generic state; we could have also built a hierarchy of states, but let's
// keep it easy As such, to ease comparision between these states, they will be stored
// as integers (otherwise, we'd have type conversion issues)
...
void Subject::Release(Observer *ob) {
  ...
  // for (list<Observer *>::iterator iter = observers.begin(); iter != observers.end() && !found;
  //      iter++)
...
void Subject::Notify() {
  ...
  for (auto iter = observers.begin(); iter != observers.end(); ++iter) {
    ...
    // because at this level, we have no knowledge of how the Subject and Observer
    // have been specialized In our application, this means a Student (observer) added
    // a course, got off waitlist (so waitlist had a removal), so update the iterator
...
class Course : public Subject {
  // over-simplified Course class inherits observer list (from Subject)
  // which will represent Students on wait-list

private:
  string title;
  // in-class initialization will be over-written with alt. constructor
  int number = 0;
  // List of Students enrolled in Course -- could alternately
  // be set to nullptrs in constructor
  Student *students[MAXSTUDENTS] = {};
  int totalStudents = 0;

public:
  Course(const string &title, int num) : number(num) {
    // note: totalStudents initialized in in-class initalization above to 0
    this->title = title;
    // or use different parameter name and set in member init list
    // note: in-class initialization is in-lieu of below:
    // for (int i = 0; i < MAXSTUDENTS; i++) students[i] = nullptr;
  }
  // Note: destructor body shown to provide a place to add the more
  // work required -- see note below (hence why not = default)
...
private:
  ...
  Course *waitListedCourse = nullptr; // Course we'd like to take - we're on the waitlist -- this is
                                      // our Subject in specialized form
...
// Remember, gpa, currentNumCourses and waitListedCourse have already been set
// with in-class initialization Also remember, the default base class constructors
// will also be called implicitly
Student::Student() : studentId(to_string(numStudents + 100) + "Id") {
  // Note: courses have been nulled out with in-class initialization. Alternatively,
  // do this here:
  // for (int i = 0; i < MAXCOURSES; i++) courses[i] = nullptr;
  // again, remember, waitListedCourse = nullptr; has already been set using in-class initialization
  numStudents++;
}

// Alternate constructor member function definition
// Note, the base Observer() default constructor is called automatically - thus it
// is not needed in init list below (shown as a reminder that it's called)
Student::Student(const string &fn,
  ...
  // Note: courses have been nulled out with in-class initialization.
  // Alternatively, do this here:
  // for (int i = 0; i < MAXCOURSES; i++) courses[i] = nullptr;
...
// is not needed in init list below (shown as a reminder that it's called)
Student::Student(
    const string &fn, const string &ln, char mi, const string &t, float avg, const string &id)
    : Person(fn, ln, mi, t), // Observer(),
      gpa(avg), studentId(id), currentNumCourses(0) {
...
void Student::Update() {
  // Course state changed to 'Open' or 'SpaceAvail' so we can now add it.
  if ((waitListedCourse->GetState() == CourseState::OpenForEnrollment) ||
      (waitListedCourse->GetState() == CourseState::NewSpaceAvailable)) {
    // if success in Adding (I mean, it could have failed for several reasons)
    if (AddCourse(waitListedCourse)) {
      cout << GetFirstName() << " " << GetLastName() << " removed from waitlist and added to "
           << waitListedCourse->GetTitle() << endl;
      // set Observer's state to 'AddSuccess' (e.g. we were able to add Course)
      SetState(StudentState::AddSuccess);
      // Remove Observer (Student = this) from Subject (Course's waitlist)
      waitListedCourse->Release(this);
      // Set our link to Subject to Null; that is, we no longer have
      // a waitListedCourse (we added it!)
      waitListedCourse = 0;
    }
  }
...
int main() {
  ...
  // Instantiate Students and select a course they'd like to be on the
  // waitlist for -- to be added as soon as registration starts
  ...
  // Students on wait-list will automatically be Added (as room allows)
  c1->Open();
  c2->Open();
  c3->Open();
  ...
  // Now that registration is open, Students can add Courses. Should a Course be
  // full, the Student will be added to the Course waitlist Note: Course inherits
  // from Subject which keeps a list of Students interested in adding course
  s1.AddCourse(c2);
  s2.AddCourse(c2);
  s4.AddCourse(c2);

  s5.AddCourse(c2);
  // When Course state changes to "available space in class", Students on waitlist
  // (Observers) are Notified to try to Add the Course. Of course, successful
  // adding only happens as space allows. The rest stay on waitlist.
  ...
  // Implement DropCourse(). When a Student Drops a course, this event will cause
  // Course state to become "Available Space in Course". Notify() will then be called
  // on the Course (Subject), which will call Update() the list of Observers (students
  // on the waitList). This Update() will indirectly allow waitlisted Students, if any,
  // to now Add the course. Lastly, in DropCourse, remove course from Student's courselist.
