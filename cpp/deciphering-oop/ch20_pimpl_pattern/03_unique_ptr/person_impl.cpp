...
// Purpose: To illustrate pImpl pattern, this version uses a unique pointer as
// association from Person to PersonImpl Note: destructor in Person no longer has
// to delete the associated implementation (managed by unique pointer)
...
// Remember, we're using system-supplied default constructor, but if we wrote it:
...
/* Person::PersonImpl::PersonImpl(const Person::PersonImpl &p) : firstName(p.firstName),
...
void Person::PersonImpl::ModifyTitle(const string &newTitle) { title = newTitle; }
...
void Person::PersonImpl::IsA() const { cout << "Person" << endl; }

void Person::PersonImpl::Greeting(const string &msg) { cout << msg << endl; }
...
// copy constructor -- in member init list, call PersonImpl copy constructor
// with new nested object creation
Person::Person(const Person &p) : pImpl(new PersonImpl(*(p.pImpl))) {
  ...
  // pImpl = new PersonImpl(*(p.pImpl));  // call PersonImpl copy constructor with
  // new nested object creation
}

// Note: Person destructor no longer needs to delete pImpl member
// and hence this simply can be the default destructor! Prototyped with virtual, but set with
// =default in implementation file
Person::~Person() = default; // delete on pImpl no longer required

const string &Person::GetFirstName() const { return pImpl->GetFirstName(); }

const string &Person::GetLastName() const { return pImpl->GetLastName(); }

const string &Person::GetTitle() const { return pImpl->GetTitle(); }

char Person::GetMiddleInitial() const { return pImpl->GetMiddleInitial(); }
