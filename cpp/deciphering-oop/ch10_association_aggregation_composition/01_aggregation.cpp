//
// Purpose: To illustrate aggregation (composition) of objects modelled via a data
// member.For each student, a unique id exists. We'll assume that an id always exists
// for a given student. Both the student and id are implemented as objects. The Student
// object will contain an embedded object for id to model the aggregation (composition).

// The part does not have a reason to exist without the whole.
...
public:
  ...
  // Remember, we get the default copy constructor and destructor, even without
  // these prototypes. Hence, they are commented out.
  // Id(const Id &) = default;
  // ~Id() = default;
...
// We're using default copy constructor, but if you wrote it,
// this is what it would look like: Remember to prototype it then too!
...
Student::Student()
    : studentId(to_string(numStudents + 100) + "Id") // set studentId with manufactured id
{
  numStudents++;
}
...
const string &Student::GetStudentId() const { return studentId.GetId(); }
