...
struct comparison {
  // This struct represents a ‘functor’ that is, a ‘function object’
...
int main() {
  ...
  // we'll compare to a range-for below This method shows how we explicitly declare
  // the iterator and access each element in the pair. The next method further below
  // is simpler, but let's appreciate the evolution here.
  ...
  // decomposition in []'s You may need to compile with a special flag to get the
  // decomposition (breaking from first, second) w certain compilers For example:
  // g++ -std=gnu++1z Chp14-Ex8.cpp
