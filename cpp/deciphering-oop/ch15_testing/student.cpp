      ...
      studentId(s.studentId) // take over source objects's resource
{
  ...
  // for ptr data member, destination data member takes over source data member's memory (done in
  // member init list above)
  s.studentId = nullptr; // then null out source pointer data member
  numStudents++; // It is a design choice whether you want to increment the counter here. After all,
                 // the source object is now a shell of an instance with zeroed out values.
                 // Nonetheless, the source instance still exists in memory.
}

// destructor definition
Student::~Student() {
  cout << "Student destructor" << endl;
  // On some older compilers, below cast is necessary (delete complains with a const char * so we
  // cast constness away) delete const_cast<char *>(studentId); On newer compilers, below is fine!
  delete[] studentId;
  numStudents--;
}
...
int Student::GetNumberStudents() { return numStudents; }
