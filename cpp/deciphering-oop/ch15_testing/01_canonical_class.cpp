...
// Explicit default constructor, copy constructor and overloaded assignment operators.
...
private:
  ...
  char *title =
      nullptr; // we'll keep this data member as a char * to demonstrate deep copy/assignment
...
// Remember, we're using system supplied default constructor. Data members
// middleInitial and title will be set using in-class initialization.
...
Person::Person(const Person &p)
    : firstName(p.firstName), lastName(p.lastName), middleInitial(p.middleInitial) {
...
// Then null out right hand objects pointers (we've relinquished those members).
// Non-pointer data is just copied. Non-pointer data members (such as string or single char)
// are simply copied (string ensures a deep copy with =)
      ...
      title(p.title) // note destination pointer takes over source pointer's memory
{
  ...
  p.title = nullptr;      // null out source pointer since memory should not be shared
                          // (it now belong to destination object)
}
...
Person &Person::operator=(Person &&p) {
  ...
  if (this != &p) {
    ...
    // Non-pointer data members can be copied easily via assignment and then set to a zeroed or
    // empty type value
    ...
    middleInitial = p.middleInitial;
    p.middleInitial = '\0';
    title = p.title; // with ptr data member, this is a pointer assignemt - destination takes over
                     // source object's memory
    p.title = nullptr;
  }
  return *this; // allow for cascaded assignments
}
...
private:
  ...
  // Again, we have one pointer data member to demonstrate deep copy/assignment
...
public:
  ...
  // In the derived class, the keyword virtual is optional for overridden (polymorphic)
  // methods, as is the keyword "override" Currently, "override" is recommended for
  // internal documentation, however "virtual" is not recommended
...
// Notice that data members using in-class initialization (above), will be set for those
// members not in the member init list. However, those that can not be easily set with
// in-class initialization (such as static numStudents), we set below in method. Recall
// that member objects (strings) will be default constructed, so no additional init is
// necessary (if an empty string is our goal)
Student::Student() {
  // studentId is temporarily implemented as a char * to demonstrate copy and move copy
  // ctor and op=, etc. more thoroughly Recall, the chars pointed to must be treated as
  // const, but the pointer itself is not. Hence, we set it below to a unique id
  // (based upon numStudents counter + 100), concatenated with the string "Id" using
  // a temp (actually two of them) Remember, string member currentCourse will be default
  // constructed with an empty string - it is a member object Also, remember to dynamically
  // allocate memory for any pointer data members here (not needed in this example)
...
Student::Student(const Student &s) : Person(s), gpa(s.gpa), currentCourse(s.currentCourse) {
  ...
  // deep copy of pointer data member (use a temp since data is const and
  // can't be directly copied into)
...
// Then null out right hand objects pointer members (we've relinquished those members).
// Non-pointer data is just copied.
Student::Student(Student &&s)
    : Person(move(s)), // make sure we call base class Move copy constructor
      ...
      studentId(s.studentId) // take over source objects's resource
{
  ...
  // clear out original object's value for ptr data member, destination data member
  // takes over source data member's memory (done in member init list above)
  s.currentCourse.clear();
  s.studentId = nullptr; // then null out source pointer data member
  // It is a design choice whether you want to increment the counter here.
  // After all, the source object is now a shell of an instance with zeroed
  // out values. Nonetheless, the source instance still exists in memory.
  numStudents++;
}
...
Student::~Student() {
  ...
  // On some older compilers, below cast is necessary (delete complains with a
  // const char * so we cast constness away) delete const_cast<char *>(studentId);
  // fix cast On newer compilers, below is fine!
  delete[] studentId;
  numStudents--;
}
...
int main() {
  ...
  Person p3 =
      move(p2); // this is an initialization (not assignment) and calls move copy constructor
  p2.Print();
  p3.Print();
