...
// Remember, we're using system supplied default constructor. Data members middleInitial and title
// will be set using in-class initialization.
...
// Then null out right hand objects pointers (we've relinquished those members). Non-pointer data is
// just copied. Non-pointer data members (such as string or single char) are simply copied (string
// ensures a deep copy with =)
      ...
      title(p.title) // note destination pointer takes over source pointer's memory
{
  ...
  p.title = nullptr; // null out source pointer since memory should not be shared (it now belong to
                     // destination object)
}
