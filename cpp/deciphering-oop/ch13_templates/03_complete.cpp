...
template <class Type> class LinkList; // forward declaration with template preamble
...
// LinkList should only be extended as a protected or private base class;
// it does not contain a virtual destructor. It can be used as-is, or as
// implementation for another ADT.
...
template <class Type> void LinkList<Type>::InsertAtFront(Type *theItem) {
...
template <class Type> LinkListElement<Type> *LinkList<Type>::RemoveAtFront() {
...
template <class Type> void LinkList<Type>::Print() const {
  if (!head)
    cout << "<EMPTY>" << endl;
