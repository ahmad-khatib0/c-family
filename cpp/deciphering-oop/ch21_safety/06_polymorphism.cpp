...
// Illustrating use of choosing one of: virtual, override and final as
// a guideline essential for readability (safety)
...
public:
  ...
  // The copy constructor prototype with =default is unnecessary - we get this by automatically.
  // Hence, it is commented out. Person(const Person &) = default;  // copy constructor
  ...
  // Virtual functions will not be inlined since their method must be determined at run time using
  // v-table.
...
// Here's how it would look if you did choose to provide one (and also chose not to use in-class
// initialization)
/*
Person::Person() : middleInitial('\0')
{
   // Remember, string members are automatically initialized to empty with the default string
constructor
...
// Simple destructor written ourselves, just so we can trace the destructor chain using cout
// statements
Person::~Person() {
...
public:
  ...
  ~Student() override;      // This is a virtual destructor; override indicates 'entry' point in
