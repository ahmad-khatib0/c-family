...
// Implemented as a cooperating pair -- Singleton and SingletonDestroyer -- to make
// the Singleton This ensures proper destruction because the static non-pointer member
// SingletonDestroyer contains a link to the Singleton and, through association, forces
// its destruction. This happens when the static SingletonDestroyer is released. Otherwise,
// we'd be counting on someone to remember to delete Singleton which presents an issue --
// many may point to the Singleton. Also, as the Singleton creates itself when needed,
// the Singleton can (thourgh its pairing w SingletonDestroy) be in charge of its own
// destruction Nonetheless, we will add checks for single destruction in the rare case
// a Client deletes our Singleton directly.

// Through proper deletion of the Singleton in the SingletonDestroyer destructor, we
// see that President, Singleton, and Person destructors are each invoked as they
// contribute to the complete President object.
...
// Singleton is designed to be mixed-in to a class desiring a Singleton using
// inheritance Note: static Singleton *instance(); method won't be included in
// this class for inheritance-model
// implementation of Singleton.
//       Target class will implement this method. If this was a stand-alone Singleton
//       class, you'd include prototye here instead.
class Singleton {
protected:
...
protected:
  ...
  } // virtual because we'll inherit from Singleton
};
...
SingletonDestroyer::~SingletonDestroyer() {
  // If Singleton has been directly deleted (rare), its destructor will have
  // Nulled out SingletonDestructor's theSingleton member
  if (theSingleton == nullptr)
    ...
    delete theSingleton;
    // no need to Null out SingletonDestroyer's pointer to theSingleton
    // after delete theSingleton;
...
Person::~Person() { cout << "Person destructor" << endl; }
...
public:
  ~President() override;
  // { destroyer.setSingleton(nullptr); cout << "President destructor" << endl; }
...
President::President(const string &fn, const string &ln, char mi, const string &t)
    : Person(fn, ln, mi, t), Singleton() {}

President::~President() {
  destroyer.setSingleton(nullptr);
  // Necessary for the rare case that a Singleton is explicitly deleted.
  // The SingletonDestructor
  cout << "President destructor" << endl;
  // destructor will check this member to see if it's NULL before deallocating Singleton
}

President *President::instance(const string &fn, const string &ln, char mi, const string &t) {
  // If we have not yet allocated the Singleton
  if (theInstance == nullptr) {
    ...
    cout << "Singleton previously created. Returning existing singleton" << endl;
  return dynamic_cast<President *>(theInstance); // cast necessary because President has been stored
                                                 // as a Singleton in theInstance
}
...
int main() {
  ...
  // Singleton is so long-lived that it is appropriate for it to be deleted at end of
  // the application. Many users will have links to Singleton. Ideally, none of them
  // should delete it. The SingletonDestructor will do so. Just as Singleton allocates
  // itself internally, so will it destruct on its own (with help from its paired Destructor)
  // However, this code is written such that if someone does delete p1; then the
  // SingletonDestructor will recognize this and not doubly delete the Singleton
