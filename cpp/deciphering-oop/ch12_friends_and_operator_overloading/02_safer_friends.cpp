//
// Purpose: To illustrate a simple encapsulated LinkList, utilizing a friend class.
// A friend class is one in which all of that class's member functions are friend
// functions of the class who grants them friendship.  Friend functions of
// LinkListElement are those which have access to LinkListElement members as if they
// were within the scope of the LinkListElement class (as well as within their own class scope).
// Friend functions are sometimes necessary for tightly coupled (non-related) classes,
// when it is otherwise inappropriate to universally provide selected functions in the
// public access region.
// Friend functions are also beneficial when used in conjunction with operator overloading.
// NOTE: LinkListElement and LinkList are not intended to be extended with public
// inheritance -- they do not contain virtual destructors.
// Instead, LinkList is meant to be used on its own or to serve as a private or protected
// base class to provide the underlying implementation for other ADTs, such as Queue.
...
public:
  ...
  // in class init will set data and next
...
// If we chose to write the default constructor (versus in-class initialization),
// it might look like this (or use mbr init list)
/* LinkList::LinkList() { head = tail = current = nullptr; } */
...
void LinkList::Print()
{
   ...
   while (current)
   {
      Item output = *(static_cast<Item *>(current->GetData()));  // localize the output temporary
variable cout << output << " "; current = current->GetNext();
   }
   cout << endl;
}
*/
