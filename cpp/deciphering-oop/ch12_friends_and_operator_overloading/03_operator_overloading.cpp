//
// Purpose:  To illustrate operator overloading, both as member
// functions and non-member functions.
//
// Note: To fully demonstrate overloading operator= (with a deep assignment), we need
// one data member that is a pointer With that in mind, title will be a char *
// (rather than our preferred string) so that we can demonstrate this idea.
...
public:
  ...
  // non-member friend function for operator+ so operands can be associative
  friend Person &operator+(const string &, Person &);

  // if we want to disallow assignment
...
Person &Person::operator=(const Person &p) {
  ...
  if (this != &p) {
    ...
    // Note: a pointer is used for title to demo the necessary steps to implement
    // a deep assignment (Otherwise, we would implement title using string)
