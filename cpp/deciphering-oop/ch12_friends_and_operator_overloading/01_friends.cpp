...
// This program shows simple syntax and example access allowed in the body
// of Id::SetStudent(Student *);
...
private:
  ...
  const string &GetId() const {
    return idNumber;
  } // provide private access method for a friend scope usage
...
Student::Student(const string &fn,
                 ...
                 const string &course,
                 const string &id)
    : Person(fn, ln, mi, t), gpa(avg), currentCourse(course), studentId(new Id(id)) {
  studentId->SetStudent(this); // create back link - note forward link in member init list
  numStudents++;
}

Student::Student(const Student &s)
    : Person(s), gpa(s.gpa), currentCourse(s.currentCourse), studentId(new Id(s.GetStudentId())) {
...
const string &Student::GetStudentId() const {
  // return studentId->idNumber;  // it's more polite to use the private
  // access method than grab the member directly
  return studentId->GetId();
  // note GetId() is private, but we're in the scope of a friend class
}

void Id::SetStudent(Student *s) {
  student = s;
  // access below just to demonstrate friends -- that is, the student can
  // access its private gpa here
