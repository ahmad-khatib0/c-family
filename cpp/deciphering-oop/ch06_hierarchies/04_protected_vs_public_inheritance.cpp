...
// User supplied default constructor is not necessary due to in-class
// initialization. Here is what it would look like (or use mbr init list) if you
// opted not to include in-class initialization
/*
LinkList::LinkList()
{
...
// Print written as a const method using a local variable to traverse list
// (Preferred)
...
public:
  // Constructor prototype shown below is not needed; we get default without
  // prorotype (since there are no other ctors) Shown in comments to show
  // optional use of =default to get default ctor Queue() = default;   // will
  // call LinkList() constructor Destructor prototype is needed (per virtual
  // keyword)
  ...
  Item *Dequeue();
  // It is necessary to redefine these operations--LinkList is a protected base
  // class
...
Item *Queue::Dequeue() {
  ...
  front = RemoveAtFront();
  Item *item = new Item(
      *(static_cast<Item *>(front->GetData()))); // make copy of front's data
  delete front;
  return item;
}
...
public:
  // Constructor prototype shown below is not needed; we get default without
  // prorotype (since there are no other ctors) Shown in comments to show
  // optional use of =default to get default ctor PriorityQueue() = default; //
  // will call Queue() default constructor Destructor prototype is needed is not
  // needed for overridden virtual destructor, hence commented out
  // ~PriorityQueue() override = default; // we'll discuss override in Chp 7

  // Here, we specify the pubilc interface which PriorityQueue instances may
  // utilize. Since we are inherited from Queue publicly, we inherit the public
  // interface from Queue.  Since Queue is inherited protectedly from LinkList,
  // the public interface from LinkList which is considered protected to Queue
  // is also considered protected to PriorityQueue, so these operations are
  // available within the scope of PriorityQueue.  Note that these operations
  // would not be available within the scope of PriorityQueue if Queue were
  // inherited privately from LinkList.
