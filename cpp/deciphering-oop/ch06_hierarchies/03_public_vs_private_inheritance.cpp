...
// If we chose to write the default constructor ourselves (rather than use
// in-class initialization), it might look as below (or use the mbr init list)
/*
LinkList::LinkList()
{
...
public:
  // Constructor and destructor prototypes shown below are not needed; we get
  // both without these prototypes! Since no other constructor exists, we get
  // Stack() without any prototype. Same for destructor. Shown in comments to
  // show how optional prototyping can be done with =default Stack() = default;
  // // will call LinkList() default ctor ~Stack() = default;
  ...
  Item *Pop();
  // It is necessary to redefine these operations--LinkList is a private base
  // class
...
Item *Stack::Pop() {
  ...
  top = RemoveAtFront();
  Item *item =
      new Item(*(static_cast<Item *>(top->GetData()))); // copy top's data
  delete top;
  return item;
}
